@extends('backend.app')
@section('title', 'নতুন সহায়তাকারী যোগ করুন')
...
<div class="container-fluid">
    <div class="card p-4">
        <form action="{{ route('admin.helpers.store') }}" method="POST">
            @csrf

            <div class="mb-3">
                <label for="name" class="form-label">সহায়তাকারীর নাম <span class="text-danger">*</span></label>
                <input type="text" placeholder="সহায়তাকারীর নাম লিখুন" name="name" id="name" class="form-control" value="{{ old('name') }}" required>
                @error('name')<small class="text-danger">{{ $message }}</small>@enderror
            </div>

            <div class="d-flex justify-content-end">
                <a href="{{ route('admin.helpers.index') }}" class="btn btn-secondary me-2">বাতিল</a>
                <button type="submit" class="btn btn-primary">সহায়তাকারী যোগ করুন</button>
            </div>
        </form>
    </div>
</div>
