        ...
        <form action="{{ route('admin.costs.update', $cost->id) }}" method="POST">
            ...
            <div class="row mb-3">
                <div class="col-md-4">
                    ...
                    <input type="date" name="date" id="date" class="form-control" value="{{ old('date', $cost->date) }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <select name="cost_source_id" id="cost_source_id" class="form-select" required>
                        ...
                        @foreach($sources as $source)
                            <option value="{{ $source->id }}" {{ old('cost_source_id', $cost->cost_source_id) == $source->id ? 'selected' : '' }}>
                                {{ $source->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="amount" id="amount" class="form-control" value="{{ old('amount', $cost->amount) }}" required>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" id="description" class="form-control" rows="3">{{ old('description', $cost->description) }}</textarea>
                @error('description')<small class="text-danger">{{ $message }}</small>@enderror
            </div>

            <div class="d-flex justify-content-end">
                <a href="{{ route('admin.costs.index') }}" class="btn btn-secondary me-2">বাতিল</a>
                <button type="submit" class="btn btn-primary">খরচ হালনাগাদ করুন</button>
            </div>
        </form>
    </div>
</div>
