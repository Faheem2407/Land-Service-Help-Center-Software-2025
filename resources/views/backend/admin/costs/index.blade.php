...
<div class="container-fluid">
    <div class="card p-4">
        <div class="d-flex justify-content-between mb-3">
            <h5 class="fw-semibold">সকল খরচ</h5>
            <a href="{{ route('admin.costs.create') }}" class="btn btn-primary btn-lg">নতুন খরচ যোগ করুন</a>
        </div>
...
$(function () {
    ...
    const table = $('#data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.costs.index') }}",
        columns: [
            { data: 'DT_RowIndex', name: 'DT_RowIndex', orderable: false, searchable: false },
            { data: 'date', name: 'date' },
            { data: 'cost_source', name: 'cost_source', orderable: false, searchable: false },
    ...
    window.showDeleteConfirm = function(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let url = '{{ route("admin.costs.destroy", ":id") }}'.replace(':id', id);
                $.ajax({
                    url: url,
                    type: 'DELETE',
                    ...
                    error: function() { toastr.error('কিছু ভুল হয়েছে।'); }
                });
            }
        });
    }
